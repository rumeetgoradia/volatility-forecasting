...
from models.rf_expert import RandomForestWrapper
import joblib


class PrecomputedExpert(nn.Module):
    """
    Lightweight wrapper that serves cached forecasts keyed by datetime. Useful for
    integrating foundation model outputs (Chronos/TimesFM/Kronos) into the MoE without
    running those heavy models during training.
    """
    ...
    def __init__(
        ...
        self.fallback = fallback if fallback is not None else (float(np.median(finite_vals)) if len(finite_vals) else 0.0)
    ...
    def forward(self, x: torch.Tensor, timestamps=None):
        ...
        for ts in ts_iter:
            ...
            ts_key = ts_key.tz_localize(None) if hasattr(ts_key, "tz_localize") else ts_key
            pred_val = self.lookup.get(ts_key, self.fallback)
            preds.append(float(pred_val))

        pred_tensor = torch.tensor(preds, device=x.device, dtype=torch.float32).unsqueeze(1)
        return pred_tensor
...
class MixtureOfExperts(nn.Module):
    ...
    def forward(self, x, return_weights: bool = False, timestamps=None, regime: Optional[torch.Tensor] = None):
        batch_size = x.size(0)
        ...
        if self.use_regime_feature and regime is not None:
            if torch.is_tensor(regime):
                regime_tensor = regime.to(gating_input.device).float().view(batch_size, -1)
            else:
                regime_tensor = torch.tensor(regime, device=gating_input.device).float().view(batch_size, -1)
...
def load_expert_models(config: dict,
                      ...
                      input_size: int,
                      device: str = 'cpu',
                      feature_cols: Optional[List[str]] = None) -> Dict[str, Dict[str, nn.Module]]:
        ...
        for expert_name in expert_configs:
            if expert_name == 'har_rv':
                ...
                if model_path.exists():
                    ...
                    instrument_experts['rf'] = RandomForestWrapper(rf_model, feature_indices=feature_indices)

            elif expert_name in ('kronos_mini', 'chronos_fintext', 'timesfm_fintext', 'chronos2'):
                ...
                if pred_file.exists():
                    ...
                    instrument_experts[expert_name] = PrecomputedExpert(
                        ...
                        allowed_splits=None,  # allow train/val/test if present
                    )
...
class HARRVWrapper(nn.Module):
    def __init__(self, har_model: HARRV, feature_cols: Optional[List[str]] = None):
        ...
        self.required_cols = getattr(har_model, "feature_cols", ["RV_H1", "RV_H6", "RV_H24"])
    ...
    def forward(self, x, timestamps=None):
        if isinstance(x, torch.Tensor):
            ...
            if len(self.col_indices) == len(self.required_cols):
                ...
                # Fallback: use first columns if indices unavailable
                x_df = pd.DataFrame(x_np[:, : len(self.required_cols)], columns=self.required_cols)
