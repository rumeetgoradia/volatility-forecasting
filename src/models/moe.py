...
class MixtureOfExperts(nn.Module):
    ...
    def forward_with_regime(self, x, timestamps=None, regime=None):
        ...
        if hasattr(self.gating, 'forward_with_regime'):
            gating_weights, regime_logits = self.gating.forward_with_regime(gating_input)
        else:
            gating_weights = self.gating(gating_input)
            regime_logits = None
...
def load_expert_models(config: dict,
    ...
    for instrument in instruments:
        ...
        inst_train_df = pd.read_parquet("data/processed/train.parquet")
        inst_train_df = inst_train_df[inst_train_df["Future"] == instrument]
